...
for m in list_month:
    ...
    for period in list_period: 
        ...
        for i in range(0,len(list_rcp45)):
            ...
            data = var.where(MASK.sftlf == 100 ).mean(dim=('y', 'x'))
  
            globals()['flattened_list_'+period].append(data.values)        
    ...
    plt.savefig('K:/PROJETS/PROJET_OUTAOUAIS/VI_MONTH_'+m+'_Mean_tasmax_BV.png', bbox_inches='tight', format='png', dpi=1000)
    plt.show()  
    plt.close()

fig = plt.figure(figsize=(28,16))   
ax = plt.axes(projection=ccrs.PlateCarree())
ax.set_extent([-90,-60,40,60])
...
ax.gridlines() 
mm = ax.scatter(lonM.values, latM.values, s=200,marker='.', c='red',label='CORDEX grid point')          
...
plt.savefig('K:/PROJETS/PROJET_OUTAOUAIS/CORDEX_BOX.png', bbox_inches='tight', pad_inches=0.1)
plt.show()  
plt.close()
