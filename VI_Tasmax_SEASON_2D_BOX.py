...
rep1='K:/PROJETS/PROJET_CORDEX/CORDEX-NAM44/ANOMALIE_SAISON_INTER_ANNUEL/'
variable_in = 'Mean_tasmax'

list_period = ['2011-2100']
...
list_saison = ['DJF','MAM','JJA','SON']
for s in list_saison:
    df_rcp45 = []
    matrix_45 = []
    for period in list_period: 
        globals()['flattened_list_'+period] = []
        for i in range(0,len(list_rcp45)):
            filename= rep1 + path_rcp45[i] + '/' + period + '/' + variable_in + '/anomalie_' +  list_rcp45[i] +  '_' + variable_in + '_' + period  + '_1971-2000_' + s + '.nc'   
    ...
    for period in list_period: 
        ...
        for i in range(0,len(list_rcp85)):
            filename= rep1 + path_rcp85[i] + '/' + period + '/' + variable_in + '/anomalie_' +  list_rcp85[i] +  '_' + variable_in + '_' + period  + '_1971-2000_' + s + '.nc'    
            nc = netCDF4.Dataset(filename)
            var = nc.variables[variable_in][:]  
            lats = nc.variables['lat'][:]; lons = nc.variables['lon'][:]
            #lon2d, lat2d = np.meshgrid(lon, lat)
    ...
    for i in range(0,len(list_histo)):
        filename= rep1 + path_histo[i] + '/' + variable_in + '/1971-2000/anomalie_' +  list_histo[i] +  '_' + variable_in + '_1971-2000_1971-2000_' + s + '.nc'  
        nc = netCDF4.Dataset(filename)
        var = nc.variables[variable_in][:]  
        lats = nc.variables['lat'][:]; lons = nc.variables['lon'][:]
    ...
    if s == 'DJF':        
        title='Seasonal change in daily maximum temperature: (1971-2100) compared with normal (1971-2000)\n Winter season: December - January - February '
    elif s == 'MAM':        
    ...
    plt.title(title, fontsize=20, color='black', weight='semibold')
    ...
    plt.savefig('K:/PROJETS/PROJET_CLIMHUNOR/figures/Mean_tasmax/VI_SEASON_'+s+'_Mean_tasmax.png', bbox_inches='tight', format='png', dpi=1000)
    plt.show()  
    plt.close()
