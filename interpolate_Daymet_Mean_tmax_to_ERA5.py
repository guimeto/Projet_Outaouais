...
import numpy as np
#for interpolation
from scipy.spatial import cKDTree# -*- coding: utf-8 -*-
"""
Created on Tue Apr 14 13:53:00 2020
Script to interpolate Daymet grid to ERA5 grid using cKDTree from scipy
@author: guillaume
...
path_source = './Daymet_Month_Indices/Mean_Tmax/'
...
source = xr.open_dataset(path_source + 'Daymet_v3_Monthly_Mean_Tasmax_199001_OUTAOUAIS.nc')
...
def interpolate(source, target, d, inds):         
    w = 1.0 / d**2
    air_idw = np.sum(w * source.tmax.values.flatten()[inds], axis=1) / np.sum(w, axis=1)
...
for year in range(yi,yf+1):
    for i in range (1,13○,1):  
        ...
        data_set = xr.Dataset( coords={'lon': ([ 'lon'], lon_target),
                                             'lat': (['lat',], lat_target)})
        data_set["Mean_tasmax"] = (['lat', 'lon'],  air_idw)
