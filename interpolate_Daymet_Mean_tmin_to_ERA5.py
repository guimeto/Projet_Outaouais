...
path_source = './Daymet_Month_Indices/Mean_Tmin/'

variable = 'Monthly_Mean_Tasmin'
yi = 1990
yf = 2019
...
source = xr.open_dataset(path_source + 'Daymet_v3_Monthly_Mean_Tasmin_199001_OUTAOUAIS.nc')
...
def interpolate(source, target, d, inds):         
    w = 1.0 / d**2
    air_idw = np.sum(w * source.tmin.values.flatten()[inds], axis=1) / np.sum(w, axis=1)
...
for year in range(yi,yf+1):
    for i in range (1,13,1):  
        ...
        data_set = xr.Dataset( coords={'lon': ([ 'lon'], lon_target),
                                             'lat': (['lat',], lat_target)})
        data_set["Mean_tasmin"] = (['lat', 'lon'],  air_idw)
